...
def merge_overlapping_boxes(boxes, gap=0):
    """
    Gộp các box chồng lấn HOẶC nằm gần nhau (khoảng cách <= gap px) thành 1 box lớn.
    gap=0: chỉ gộp box thực sự chồng nhau (hành vi cũ).
    gap>0: gộp cả box liền kề, cách nhau trong khoảng gap px -> dùng để gộp
    các bảng/hàng title-block đứng sát nhau thành 1 vùng crop duy nhất.
    """
    if not boxes:
        return []
    # Giãn mỗi box thêm `gap` mỗi phía để kiểm tra "đủ gần" bằng phép giao nhau,
    # nhưng box kết quả cuối cùng vẫn tính trên toạ độ GỐC (không cộng dồn gap).
        ...
        for i in range(len(rects)):
            ...
            for j in range(i + 1, len(rects)):
                if used[j]:
                    continue
                a1, b1, a2, b2 = rects[j]
                # "đủ gần để gộp": khoảng cách 2 box (mở rộng gap) vẫn giao nhau
                if (x1 - gap) < (a2 + gap) and (a1 - gap) < (x2 + gap) and \
                   (y1 - gap) < (b2 + gap) and (b1 - gap) < (y2 + gap):
